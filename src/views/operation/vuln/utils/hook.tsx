...
//   getAllRoleList
// } from "@/api/system";
import { getVuln, addVuln, delVuln, reVuln } from "@/api/operation";
import {
  type Ref,
  h,
  ref,
...
export function useUser(tableRef: Ref) {
  ...
  function onbatchDel() {
    ...
    delVuln({ ids: getKeyList(curSelected, "vulnId") });
  ...
  async function resetForm() {
    form.vulnerabilityType = "";
    onSearch();
  }

  async function onSearch() {
    loading.value = true;
    const { data } = await getVuln(toRaw(form));
    ...
    addDialog({
      ...
      props: {
        formInline: {
          ...
          vulnerabilitySuggest: row?.vulnerabilitySuggest ?? ""
        }
      },
      width: "46%",
      draggable: true,
