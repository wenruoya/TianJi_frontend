...
const fetchVulnOptions = async () => {
  ...
  const secUserResponse = await getSecOptions();
  devUserOptions.value = devUseResponse.data.map((item: any) => ({
    label: item.name,
    value: item.userId
  }));
  secUserOptions.value = secUserResponse.data.map((item: any) => ({
    label: item.name,
    value: item.userId
  }));
...
<template>
  <el-card shadow="never">
    ...
    <div
      ref="preventLocal"
      class="w-7/8 h-[535px] border border-indigo-300 mt-3"
    >
...
function h(arg0: string, arg1: string): any { throw new Error("Function not
implemented."); }
